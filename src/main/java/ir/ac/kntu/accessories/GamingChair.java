...
public class GamingChair extends Main {
    ...
    //-----------( managing data )-----------
    public GamingChair(String name,
                       int height,
                       int price,
                       String color,
                       int numberOf,
        ...
        this.id = "GC" + DataManager.getGamingChairs().size();
    ...
    private void userPanel() {
        ...
        if (DataManager.getCurrentUser().getGamingChairs().contains(this)) {
            userOptions();
        }
        if (this.numberOf > 0) {
    ...
    private void buy() {
        if (DataManager.getCurrentUser().getWallet() < this.price) {
            ...
            DataManager.getCurrentUser().getGamingChairs().add(this);
            this.numberOf--;
    ...
    private void sellerPanel() {
        ...
        if (this.seller == DataManager.getCurrentSeller()) {
            ...
            switch (userCommand) {
                case "1" -> {
                    DataManager.getGamingChairs().remove(Integer.parseInt(this.id.
                            replace("GC", "")));
                    System.out.println("done\n\n");
